...
namespace BotProject.Web.Controllers
{
	...
	public class Apiv1Controller : Controller
	{
		...
		[HttpPost]
		public async Task<JsonResult> Chat(Message message)
		{
			...
			if (String.IsNullOrEmpty(text))
			{
				...
				_botHub.SendMessageBot(message.senderId, message.text);
		...
		private async Task<string> MessageResponse(string text,
										   string sender,
										   int botId,
										   string typeRequest)
		{
			...
			{
				// Thêm dấu tiếng việt, đã thêm ngoài client
			}
