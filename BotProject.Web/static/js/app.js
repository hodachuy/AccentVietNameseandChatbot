...
var url;
var lacviet = {
    load: function (encryptUrl, token, botId) {
        var decryptedUrl = CryptoJS.AES.decrypt(encryptUrl, "Secret Passphrase").toString(CryptoJS.enc.Utf8);
        var decryptedToken = CryptoJS.AES.decrypt(token, "Secret Passphrase").toString(CryptoJS.enc.Utf8);
        var decryptedbotId = CryptoJS.AES.decrypt(botId, "Secret Passphrase").toString(CryptoJS.enc.Utf8);
        console.log(decryptedUrl, decryptedToken, decryptedbotId)
        lacviet.initFormChat(decryptedUrl, decryptedToken, decryptedbotId);
        lacviet.initCSS(decryptedUrl);
    },
    initFormChat: function (dencryptUrl, token, botId) {
        //var decryptedUrl = CryptoJS.AES.decrypt(encryptUrl, "Secret Passphrase").toString(CryptoJS.enc.Utf8);
    ...
    initCSS: function (dencryptUrl) {
        ...
        link.href = dencryptUrl + "assets/client/css/chatbot-dialog.css";
    }
};
$('body').on('click', '.fb_dialog', function (e) {
    if ($("#dialog_iframe").hasClass("fb_customer_chat_bounce_out_v2")) {
        ...
        setTimeout(function () {
            ...
            frame.contentWindow.postMessage('init', url);
        }, 200)
    }
