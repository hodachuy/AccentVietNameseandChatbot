    ...
    public class WebhookController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> Post()
        {
            ...
            foreach (var item in value.entry[0].messaging)
            {
                ...
                {
                    await SendMessage(GetMessageTemplate(item.postback.payload, item.sender.id, botId));
                }
                else
                {
                    if(item.message.quick_reply == null)
                        await SendMessage(GetMessageTemplate(item.message.text, item.sender.id, botId));
                    else
                        await SendMessage(GetMessageTemplate(item.message.quick_reply.payload, item.sender.id, botId));
                }
            }
            return new HttpResponseMessage(HttpStatusCode.OK);
        }

        public async Task ExcuteMessage(string text, string sender, int botId)
        {
                ...
                if (fbUserDb.IsHavePredicate)
                {
                    ...
                    if(predicateName == "ApiSearch")
                    {
                        if (text.Contains("postback_card"))// nều còn điều kiện search mà chọn postback
                        {
                            ...
                            _appFacebookUser.Save();
                            await ExcuteMessage(text, sender, botId);
                        }
                        ...
                        await SendMessage(handleMdSearch.TemplateJsonFacebook);
                    }
                    if (predicateName == "Age")
                    {
                        ...
                        if (handleAge.Status)// đúng age
                        {
                            ...
                            {
                                await ExcuteMessage(handleAge.Postback, sender, botId);
                            }
                        }
                        await SendMessage(handleAge.TemplateJsonFacebook);
                    }
                    if (predicateName == "Phone")
                    {
                        ...
                        if (handlePhone.Status)// đúng số dt
                        {
                            ...
                            {
                                await ExcuteMessage(handlePhone.Postback, sender, botId);                                
                            }
                        }
                        await SendMessage(handlePhone.TemplateJsonFacebook);
                    }
                    if(predicateName == "Email")
                    {
                        ...
                        if (handleEmail.Status)// đúng email
                        {
                            ...
                            {
                                await ExcuteMessage(handleEmail.Postback, sender, botId);
                            }
                        }
                        await SendMessage(handleEmail.TemplateJsonFacebook);
                    }
                    if(predicateName == "Voucher")
                    {
                        ...
                        await SendMessage(handleMdVoucher.TemplateJsonFacebook);
                    }
                    if(predicateName == "IsVoucherOTP")
                    {
                        ...
                        await SendMessage(handleOTP.TemplateJsonFacebook);                       
                    }
                }
            ...
            if (result.Replace("\r\n", "").Trim().Contains(CommonConstants.POSTBACK_MODULE))
            {
                if (result.Contains("<module>") != true)// k phải button module trả về
                {
                    ...
                    if (idxModule != -1)
                    {
                        ...
                        {
                            await ExcuteMessage(contains, sender, botId);
                        }
                        ...
                        {
                            await ExcuteMessage(txtModule, sender, botId);
                        }
            ...
            if (result.Contains("NOT_MATCH"))
            {
                ...
                if(systemConfigVm.Count() == 0)
                {
                    ...
                    {
                        if(item.Key == result.Replace(".", String.Empty))
                        {
                            await SendMessage(GetMessageTemplateText(item.Value, sender));
                        }
                    }
                }
                ...
                AddHistory(hisVm);
                result = GetRelatedQuestionToFacebook(nameFunctionAPI, text, field, number, botId.ToString());
            ...
            if (text.Contains("postback_card"))
            {
                ...
                {
                    foreach(var item in strArrayJson)
                    {
               ...
               await SendMessage(tempJsonFacebook);
            }
            string objPostback = aimlBotResult.SubQueries[0].Template;


        }


        private JObject GetMessageTemplateText(string text, string sender)
        {
